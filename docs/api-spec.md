...
`id`는 현재 다음 값 중 하나여야 한다. `약한비`, `비옴`은 하위 호환을 위해
`우천` 시나리오와 같은 관측값을 사용한다.
...
`TELEGRAM_CHAT_ID`는 문자열, 숫자, 또는 문자열/숫자 배열을 받을 수 있다.
서버 내부에서는 문자열 배열로 정규화한다.
